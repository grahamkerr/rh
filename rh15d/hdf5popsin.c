...
void readPopsin_hdf5(int xi, int yi, Atmosphere *atmos, Geometry *geometry,
  ...
  if (strcmp(atom->ID,"H ") == 0){
      ...
      if (( H5Dclose(infile->H_popsin_varid) ) < 0) HERR(routineName);
    }
  ...
  if (strcmp(atom->ID,"CA") == 0){
      ...
      if (( H5Sclose(dataspace_id) ) < 0) HERR(routineName);
      if (( H5Sclose(memspace_id) ) < 0) HERR(routineName);
      if (( H5Dclose(infile->Ca_popsin_varid) ) < 0) HERR(routineName);
    }
  ...
  if (strcmp(atom->ID,"HE") == 0){
      ...
      if (( H5Dread(infile->He_popsin_varid, H5T_NATIVE_DOUBLE, memspace_id,
                dataspace_id, H5P_DEFAULT, atom->n[0]) ) < 0) HERR(routineName);
      if (( H5Sclose(dataspace_id) ) < 0) HERR(routineName);
      if (( H5Sclose(memspace_id) ) < 0) HERR(routineName);
      if (( H5Dclose(infile->He_popsin_varid) ) < 0) HERR(routineName);
    }
